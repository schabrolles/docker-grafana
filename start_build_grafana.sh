...
docker run --rm --hostname grafana_build -ti -p 80:80 -v $PWD/opt:/opt -v $PWD/build_script:/build_script --name grafana_build schabrolles/ubuntu_ppc64le:17.04pre /build_script/build_grafana.sh
...
cd Docker_build
docker build -t schabrolles/grafana_ppc64le$DOCKER_TAG .
